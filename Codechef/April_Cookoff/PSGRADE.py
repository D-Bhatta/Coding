""" Passing Marks Problem Code: PSGRADE
...
def compare_subjects(a, a_min):
    if a >= a_min:
        return True
    return False


def has_passed(a_min, b_min, c_min, t, a, b, c):

    passed = "NO"

    if sum((a,b,c)) >= t:
        if compare_subjects(a, a_min) and compare_subjects(b, b_min) and compare_subjects(c,c_min):
            passed = "YES"
...
for i in range(num_cases):
    a_min, b_min, c_min, t, a, b, c = map(int, input().strip().split())
    print(has_passed(a_min, b_min, c_min, t, a, b, c))
